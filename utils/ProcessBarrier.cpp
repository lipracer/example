...
template <typename DerivedT> class ProcessBarrier : public ProcessBarrierBase {
  protected:
    struct SharedHandle {
        SharedHandle()
        {
            shmid_ = shmget((key_t) kSharedMemId(), sizeof(ShareMemoryType), 0640 | IPC_CREAT);
        ...
        {
            return share_mem_->wait_count;
        }
        void setWaitingCount(size_t count)
        {
            share_mem_->wait_count = count;
        }
        void increase()
        {
            ++(share_mem_->wait_count);
        }

        void clearMemory()
        {
            setWaitingCount(0);
            for (size_t i = 0; i < sizeof(share_mem_->msg) / sizeof(share_mem_->msg[0]); ++i) {
                share_mem_->msg[i] = BarrierState::initialize;
            }
        }

        void setStartTP(int64_t pt)
        {
            share_mem_->time_point = pt;
        }
        int64_t getStartTP()
        {
            return share_mem_->time_point;
        }
        ...
        const BarrierState* getStates()
        {
            return share_mem_->msg;
        }
    ...
    {
        static std::string str = "__kAllReduceSemFlagNIII__";
        return str;
    }

    static const std::string& kAllReduceSemInitFlag()
    {
        static std::string str = "__kAllReduceSemInitFlagNIII_X__";
        return str;
    }

    static int kSharedMemId()
    {
        return ftok("/dev/null", 2);
    }
...
class BarrierServer : public ProcessBarrier<BarrierServer> {
  public:
    BarrierServer(size_t nclient) : ProcessBarrier<BarrierServer>(), nclient_(nclient)
    {
        clearState();
        ...
        share_handle_->setStartTP(std::chrono::duration_cast<duration_type>(
                                      std::chrono::steady_clock::now().time_since_epoch())
                                      .count());
        for (size_t i = 0; i < nclient_ - 1; ++i) {
            sem_post(sem_init_);
    ...
    sem_t* getInitializeSem() {
        return sem_open(kAllReduceSemInitFlag().c_str(), O_CREAT, S_IROTH | S_IWOTH, 0);
    }
...
class BarrierClient : public ProcessBarrier<BarrierClient> {
  public:
    ...
    sem_t* getInitializeSem() {
        sem_t* ret = nullptr;
        while (!ret) {
            ret = sem_open(kAllReduceSemInitFlag().c_str(), O_EXCL, S_IROTH | S_IWOTH, 0);
            std::this_thread::yield();
        }
        return ret;
    }
...
std::unique_ptr<ProcessBarrierBase> createBarrier(size_t worker_size, size_t id) {
    if (id == 0) {
